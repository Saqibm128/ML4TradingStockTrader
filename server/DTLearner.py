    ...
    def addEvidence(self, Xtrain, Ytrain):
        ...
        if Xtrain.shape[0] <= self.leaf_size:
            ...
            for i in range(Xtrain.shape[1]):
                if maxRSq < (np.corrcoef(Xtrain[:,i], Ytrain)[0, 1])**2:
                    maxInd = i
                    maxRSq = (np.corrcoef(Xtrain[:,i], Ytrain)[0, 1])**2
            ...
            if leftXtrain.shape[0] == 0 or rightXtrain.shape[0] == 0: #median was also a mode...
                self.addNode([True, np.nan, np.median(Ytrain), np.nan, np.nan])
                return;
